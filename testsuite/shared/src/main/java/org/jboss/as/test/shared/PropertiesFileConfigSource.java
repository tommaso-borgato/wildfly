...
public class PropertiesFileConfigSource implements ConfigSource {
    ...
    private Properties getPropertiesFromFile() {
        if (properties == null) {
            ...
            String filePath = System.getProperty(PROPERTIES_FILE_PROPERTY_NAME) != null ?
                    System.getProperty(PROPERTIES_FILE_PROPERTY_NAME) : System.getenv(PROPERTIES_FILE_PROPERTY_NAME.toUpperCase().replace(".", "_"));
            logger.info("Loading properties from file: " + filePath);
            if (filePath != null) {
                try (FileReader reader = new FileReader(Paths.get(filePath).toFile())) {
                    properties.load(reader);
                } catch (IOException e) {
                    throw new RuntimeException("Could not load properties from " + filePath , e);
                }
            }
        }
        return properties;
    ...
    public Set<String> getPropertyNames() {
        return getPropertiesFromFile().keySet().stream().map(o -> o.toString()).collect(Collectors.toSet());
    }
    ...
    public String getValue(String s) {
        return getPropertiesFromFile().getProperty(s);
    }
